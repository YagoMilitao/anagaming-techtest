...
const OddsList: React.FC<OddsListProps> = ({ odds, isLoading = false }) => {
    ...
    <ul className="space-y-4 mt-3">
      <AnimatePresence>
        {odds.map((odd) => {
          ...
          const bestHome = parseFloat(bestPriceHome);
          const bestAway = parseFloat(bestPriceAway);
          const isHomeBetter = bestHome >= bestAway;
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
                <div>
                  <div className="text-base text-gray-700">
                    Melhor Odd:{' '}
                    <span className={isHomeBetter ? 'font-semibold text-green-600' : 'font-semibold'}>
                      {bestOutcomeHome || '-'} ({bestPriceHome})
                    </span>
                  </div>
                  {bestBookmakerHome && (
