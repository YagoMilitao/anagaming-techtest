...
describe("sitemap", () => {
  ...
  it("should generate a sitemap with static pages when no odds data is available", async () => {
    ...
    expect(result).toEqual([{ url: baseUrl, lastModified: expect.any(Date), changeFrequency: "daily", priority: 1.0 }]);
  });
  ...
  it("should generate sitemap entries for dynamic sport pages", async () => {
    const mockOdds: Odd[] = [
      ...
      },
      {
        ...
        sport_title: "NBA",
        commence_time: "2025-06-02T10:00:00Z",
        home_team: "C",
        away_team: "D",
        bookmakers: [],
      },
  ...
  it("should combine static, dynamic sport, and dynamic event pages correctly", async () => {
    ...
    expect(result).toContainEqual(
      expect.objectContaining({ url: `${baseUrl}/sports/basketball-nba/2`, priority: 0.8 }),
    );
  });
